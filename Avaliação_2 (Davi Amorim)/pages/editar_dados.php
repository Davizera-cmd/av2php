...
if (!defined('TAMANHO_MAX_FOTO')) define('TAMANHO_MAX_FOTO', 2 * 1024 * 1024); // 2MB
if (!defined('TAMANHO_MAX_PDF')) define('TAMANHO_MAX_PDF', 5 * 1024 * 1024); // 5MB
...
function tratarUploadArquivoEdicao($arquivo, $pastaDestino, $tiposPermitidos, $tamanhoMaximo, $campoNome, $arquivoAntigo = null) {
    if (isset($arquivo) && $arquivo['error'] === UPLOAD_ERR_OK && $arquivo['size'] > 0) {
        ...
        if (move_uploaded_file($arquivo['tmp_name'], $caminhoCompletoNovo)) {
            if ($arquivoAntigo && file_exists($pastaDestino . $arquivoAntigo)) {
                @unlink($pastaDestino . $arquivoAntigo); 
            }
            return ['sucesso' => $nomeNovoArquivo];
        } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['salvar_dados'])) {
    if (!$cliente_atual) { 
        ...
        $uploadFotoNova = tratarUploadArquivoEdicao($_FILES['foto_perfil_nova'], PASTA_UPLOADS_IMG, TIPOS_PERMITIDOS_FOTO, TAMANHO_MAX_FOTO, 'Foto_de_Perfil', $nomeArquivoFotoAntigo);
        ...
        $uploadPdfNovo = tratarUploadArquivoEdicao($_FILES['documento_pdf_novo'], PASTA_UPLOADS_PDF, TIPOS_PERMITIDOS_PDF, TAMANHO_MAX_PDF, 'Documento_PDF', $nomeArquivoPdfAntigo);
        ...
        if (empty($erros_validacao)) {
            ...
            if ($nomeArquivoFotoParaSalvar !== $nomeArquivoFotoAntigo) { 
                $alteracoes_para_log[] = ['campo' => 'foto', 'antigo' => $nomeArquivoFotoAntigo, 'novo' => $nomeArquivoFotoParaSalvar];
            }
            if ($nomeArquivoPdfParaSalvar !== $nomeArquivoPdfAntigo) {
                $alteracoes_para_log[] = ['campo' => 'pdf', 'antigo' => $nomeArquivoPdfAntigo, 'novo' => $nomeArquivoPdfParaSalvar];
            }
            if ($alterar_senha) {
    ...
    <div class="container">
        <div class="form-container" style="max-width: 650px;">
            ...
            <?php if (!empty($mensagem_erro_form)): ?>
                <p class="mensagem-erro"><?php echo $mensagem_erro_form; // Já é HTML seguro se implode foi usado com htmlspecialchars nos erros ?></p>
            <?php endif; ?>
